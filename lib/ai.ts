...
export async function matchTutor(
  ...
  try {
    ...
    // Use LLM to reason about the best match
    const skills = Array.isArray(request.skill) ? request.skill : [request.skill];
    const skillsText = skills.join(', ');

    const prompt = `You are an AI assistant helping match students with tutors for the CCSF CS Tutor Squad.
...
Please:
1. Select the best matching tutor from the candidates above (return their name)
2. Provide a brief reasoning (1-2 sentences) explaining why this tutor is the best match
3. List available time slots that match the student's preferences (if any)
